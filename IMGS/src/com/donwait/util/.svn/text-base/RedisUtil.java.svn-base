...
public class RedisUtil {
	...
	static {
		...
		JedisPoolConfig poolConfig =  ContextUtil.context.getBean(JedisPoolConfig.class);
		//单机配置
		Properties properties = ConfigUtil.getProperties("db.properties");
		pool = new JedisPool(poolConfig,properties.getProperty("redis.ip"),6379,180000);
		
		//集群配置
		/*String masterName = "mymaster";
		Set<String> sentinels = new HashSet<String>();
		sentinels.add("192.168.244.149:26379");
		sentinels.add("192.168.244.150:26379");
		sentinels.add("192.168.244.151:26379");
		pool = new JedisSentinelPool(masterName, sentinels, poolConfig);*/
		/*JedisSentinelPool jedisSentinelPool = (JedisSentinelPool) pool;
		HostAndPort currentHostMaster = jedisSentinelPool.getCurrentHostMaster();*/
		
		
		//集群分片配置(实现原理是对sentinel集群分组管理,mymaster1,mymaseter2...)
	...
	public static void subscribe(String[] room, JedisPubSub pubSub){
		Jedis jedis = null ;
		try{
			jedis = (Jedis) pool.getResource();//从池中取出一个Jedis
			jedis.subscribe(pubSub , room);
			logger.info("redis connect success");
		}catch(Exception e){
			e.printStackTrace();
			logger.error("redis connection disconnect,try connect again after 3 s");
			try {
				Thread.sleep(3000);
				if(jedis != null) jedis.close();
				subscribe(room, pubSub);
			} catch (Exception e2) {
				e2.printStackTrace();
			}
		}finally{
	 ...
	 * @return 加值后的结果 
	 */  
	public static Long incr(String key,int length,int seconds){  
		Jedis jedis = null;  
		Long res = null;  
		try {  
			jedis = (Jedis) pool.getResource();  
			res = jedis.incrBy(key, length);
			jedis.expire(key, seconds);
		} catch (Exception e) {  
	 ...
	 * @return
	 */
	public static Object pileline(Function<Pipeline, Object> function){
		Jedis jedis = null;  
		Object o = null;
		try {  
			jedis = (Jedis) pool.getResource();  
			Pipeline pipelined = jedis.pipelined();
			o = function.apply(pipelined);
			pipelined.clear();
			pipelined.close();
		} catch (Exception e) {  
			e.printStackTrace();
